...
public abstract class AbstractPhotoRenderer {
      ...
      if (node instanceof TransformGroup
        ...
        ((TransformGroup)node).setTransform(new Transform3D(transformMatrix));
		  ...
		  while (enumeration.hasNext()) {
			updateTransformation((Node) enumeration.next(), transformGroupUserData, matrix);
        }
      }
    }
    // No Link parsing
  ...
  boolean intersectsDeformedNode(Node node, Bounds lightBounds,
    ...
    if (node instanceof Group) {
      ...
      Iterator<Node> enumeration = ((Group) node).getAllChildren();
	  while (enumeration.hasNext()) {
        if (intersectsDeformedNode((Node)enumeration.next(), lightBounds, transformGroupUserData)) {
          return true;
        }
      }
    }
  ...
  Transform3D getDeformation(Node node, Transform3D parentTransformation,
    ...
    if (node instanceof Group) {
      ...
      Iterator<Node> enumeration = ((Group) node).getAllChildren();
	  while (enumeration.hasNext()) {
        Transform3D transform = getDeformation((Node)enumeration.next(), parentTransformation,
