...
public class DimensionLine3D extends Object3DBranch {
  ...
  public DimensionLine3D(DimensionLine dimensionLine, Home home, UserPreferences preferences,
    ...
    //TODO: the Label3D setUserData issue stop pick reporting easily
    //PJPJ for picking
    setPickable(true);
  ...
  public void update() {
    ...
    if (dimensionLine.isVisibleIn3D()
      ...
      font.setSize((int) lengthStyle.getFontSize());
      
      //PJPJ FontMetrics is far better, but that requires a JComponent.getFontMetrics which is very complex
      float fontAscent = ((float)font.getStringBounds("[").getHeight() * 1.1f);
      float zTranslation = offset <= 0
              ? -fontAscent - 1
              : fontAscent + 1;       
      ...
      if (numChildren() == 0) {
        ...
        SimpleShaderAppearance mainLinesAppearance = new SimpleShaderAppearance();        
        ...
        mainLinesAppearance.setUpdatableCapabilities();// allow shader rebuilding, after all the edits to the appearance above
        Shape3D mainLinesShape = new Shape3D(lines, mainLinesAppearance);
        mainLinesShape.setCapability(Shape3D.ALLOW_APPEARANCE_READ);//PJPJ My caps are enforced strongly
        mainLinesShape.setCapability(Shape3D.ALLOW_GEOMETRY_READ);
        transformGroup.addChild(mainLinesShape);
        makePickable(mainLinesShape);
      ...
      Color color = dimensionLine.getColor() != null ? new Color(dimensionLine.getColor()) : Color.BLACK;
      linesColoringAttributes.setColor(new Color3f(color.getRed()/255.0f,color.getGreen()/255.0f,color.getBlue()/255.0f));
